...
D_zf_MODELS = {
    "LCDM": {'alpha': 1.0, 'a': 0.21, 'b': 0.12, 'c': 0.35, 'beta': 0.57},
    "SCDM": {'alpha': 0.80, 'a': 0.38, 'b': 0.09, 'c': 0.14, 'beta': 0.40},
    "SF_n2": {'alpha': 0.80, 'a': 0.38, 'b': 0.09, 'c': 0.14, 'beta': 0.40},
    "OCDM": {'alpha': 1.0, 'a': 0.07, 'b': 0.29, 'c': 0.48, 'beta': 0.65},
    "LWDM": {'alpha': 0.34, 'a': 0.03, 'b': 0.48, 'c': 0.49, 'beta': 0.70},
    "wCDM1": {'alpha': 0.73, 'a': 0.18, 'b': 0.17, 'c': 0.37, 'beta': 0.68},
    "wCDM37": {'alpha': 0.42, 'a': 0.04, 'b': 0.34, 'c': 0.49, 'beta': 0.68}
}
...
def calculate_D_zf(D_z, Mvir, D_zf_model='LCDM'):
    """Calculate formation redshift z_f"""
    params = D_zf_MODELS[D_zf_model]
    return D_z**params['alpha'] * params['a'] * (Mvir/1e12)**params['b'] + params['c'] * D_z**params['beta']
...
def process_halo_data(input_file, cosmology_name, z=0.0, D_zf_model='LCDM'):
    ...
    for i in range(len(Mvir)):
        D_zf = calculate_D_zf(D_z[i], Mvir[i], D_zf_model=D_zf_model)
