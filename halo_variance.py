...
def save_results(results, filename="output_variance.dat"):
    """Save results to file with mixed formatting"""
    output_data = np.column_stack([
        results['Mvir'], results['D_z'], results['sigma_M']])
    np.savetxt(
        filename,
        output_data,
        fmt=['%.6e', '%.6f', '%.6f'],
        header='Mass[M_sun/h] D(z) sigma(M,0)',
        comments='')
...
print("Mass [M_sun/h]    D(z)     sigma(M,0)")
print("-" * 50)
for i in range(min(5, len(masses))):
    print(f"{results['Mvir'][i]:12.2e}  {results['D_z'][i]:8.4f}  "
          f"{results['sigma_M'][i]:8.4f}")
# Save full results
save_results(results)
print(f"\nFull results saved to output_results.dat")