...
Usage: python halo_variance.py [mass_file.dat] [cosmology] [redshift]
"""
import numpy as np
import sys
import os
...
def save_results(results, redshift, cosmology, filename="output_variance.dat"):
    ...
    np.savetxt(
        ...
        header=f'Mass[M_sun/h] z D(z) sigma(M,0) sigma(0.5M,0) sigma(0.16M,0)',
        comments='')
...
print("Mass [M_sun/h]   z    D(z)  sigma(M,0)  sigma(0.5M,0) sigma(0.16M,0)")
print("-" * 50)
